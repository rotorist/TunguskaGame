...
public class ActionMeleeAttack : GoapAction
{
	...
	{
		CsDebug.Inst.CharLog(ParentCharacter, "Start executing melee attack " + ParentCharacter.name);
	...
	{
		CsDebug.Inst.CharLog(ParentCharacter, "Stop executing melee attack " + ParentCharacter.name);
	...
	public override bool CheckActionCompletion()
	{
		...
		{
			if(ParentCharacter.MyStatus.Intelligence < 2)
			{
				//if i'm facing enemy's back then for a probability, complete
				float facingAngle = ParentCharacter.MyAI.GetEnemyFacingAngle(ParentCharacter.MyAI.BlackBoard.TargetEnemy);
				if(facingAngle < 70 && UnityEngine.Random.value > 0.5f)
				{
					return true;
				}
			}
	...
	public void PerFrameUpdate()
	{
		if(ParentCharacter.MyAI.BlackBoard.TargetEnemy != null)
		{
			...
			else
			{
				...
				else if(dist < 5 && dist > _weaponReach && !_isEnemyRanged)
				{
