...
namespace RootMotion.FinalIK {
	...
	public class IKSolverAim : IKSolverHeuristic {
		...
		public void CurvedEnable(float rate)
		{
			_ikWeightRate = rate;
			_ikWeightState = 1;
			_isCurved = true;
		...
		protected override void OnUpdate() {
			...
			if(_ikWeightState == -1)
			{
				...
				IKPositionWeight -= _ikWeightRate * Time.deltaTime;
				if(IKPositionWeight <= 0.1f)
				{
					IKPositionWeight = 0;
				}
			}
			else if(_ikWeightState == 1)
			{
				...
				if(_isCurved)
				{
					curve = IKPositionWeight + 0.08f;
				}

				IKPositionWeight += _ikWeightRate * curve * Time.deltaTime;
				if(IKPositionWeight >= 0.2f)
				{
					IKPositionWeight = Mathf.Lerp(IKPositionWeight, 1, 9 * Time.deltaTime);
					_isCurved = false;
				}
			}
